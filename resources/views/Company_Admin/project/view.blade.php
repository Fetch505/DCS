...
<div id="wrapper">
    ...
    @section('content')
        <div id="app">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <a class="btn btn-primary btn-md pull-right" href="{{ route('project.index') }}"><i
                            class="fa fa-arrow-left" aria-hidden="true"></i> @lang('Company_Admin/dashboard.Back')</a>
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            ...
                            <div class="inline">
                                ...
                                <span
                                    style="position: absolute; left: 150px;">{{ optional($project->customer)->name ?? __('No Customer') }}
                                </span>
                            </div>
                            <br>
                            <div class="inline">
                                ...
                                <span
                                    style="position: absolute; left: 150px;">{{ $project->projectSupervisor ? $project->projectSupervisor->name : '' }}</span>
                            </div>
                            <br>
                            <div class="inline">
                                <label for="name">@lang('Company_Admin/dashboard.Inspector') :</label>
                                <span
                                    style="position: absolute; left: 150px;">{{ $project->projectInspector ? $project->projectInspector->name : '' }}</span>
                            </div>
                            <br>
                            <div class="inline">
                                <label for="name">@lang('Company_Admin/dashboard.Workers') :</label>
                                <span
                                    style="position: absolute; left: 150px;">{{ $totalWorkers ? $totalWorkers : '0' }}</span>
                            </div>
                            <br>
                            <div class="inline">
                            ...
                            <div class="inline">
                                ...
                                <span
                                    style="position: absolute; left: 150px;">{{ date('M j, Y', strtotime($project->start_date)) }}</span>
                            </div>
                            <br>
                            <div class="inline">
                                <label for="name">@lang('Company_Admin/dashboard.End Date') :</label>
                                <span
                                    style="position: absolute; left: 150px;">{{ date('M j, Y', strtotime($project->end_date)) }}</span>
                            </div>
                            <br>
            ...
            <div class="row">
                <div class="col-md-4 col-md-offset-2">
                    <button class="btn btn-danger pull-right" @click.prevent="showAllJobs = true"
                        role="button">@lang('Company_Admin/dashboard.show_all_Jobs')</button>

                    <button class="btn btn-success pull-right" @click.prevent="showAllJobs = false"
                        role="button">@lang('Company_Admin/dashboard.filter_jobs')</button>
                </div>
            </div>
            <br>
            ...
            <div class="row">
                <div class="panel panel-primary">
                    <div class="panel-heading inline">
                        ...
                        <a class="btn btn-md btn-danger pull-right" name="button"
                            href="{{ route('projectAllJobPdf', $project->id) }}"><i class="far fa-file-pdf"></i>
                            @lang('Company_Admin/dashboard.Download') PDF</a>
                    </div>
                            ...
                            <table width="100%" id="table" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach ($job->days as $key => $day)
                                        @if ($day->status == 1)
                                            <tr>
                                                <td>{{ optional($day->area)->name ?? 'N/A' }}</td>
                                                <td>{{ optional($day->user)->name ?? 'N/A' }}</td>
                                                <td>{{ $day->location['name'] ?? 'N/A' }}</td>
                                                <td>{{ optional($day->element)->name ?? 'N/A' }}</td>
                                                <td>{{ optional($day->task)->name ?? 'N/A' }}</td>
                                                <td>{{ $day->type }}</td>
                                                <td>{{ $day->mon == '1' ? 'X' : '' }}</td>
                                                <td>{{ $day->tue == '1' ? 'X' : '' }}</td>
                                                <td>{{ $day->wed == '1' ? 'X' : '' }}</td>
                ...
                <div class="panel panel-primary">
                    <div class="panel-heading inline">
                        ...
                        <div class="pull-right">
                            <a href="{{ route('project.assign-materials', ['projectId' => $project->id]) }}" name="button"
                                class="btn btn-success">@lang('Company_Admin/dashboard.Add') @lang('Company_Admin/dashboard.Material')</a>
                            <a href="{{ route('project.material-transactions', ['projectId' => $project->id]) }}"
                                name="button" class="btn btn-success">@lang('Company_Admin/dashboard.Transactions') </a>
                        </div>
                    </div>
                                ...
                                @foreach ($project->materials as $key => $material)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <a
                                                    href="{{ route('material.assign-users', ['projectId' => $project->id, 'materialId' => $material->id]) }}"><i
                                                        class="fa fa-plus" style="color:blue;"></i></a>
                                                <a href="#" title="View Details" class="view-icon" data-toggle="modal"
                                                    data-target="#assignedUsersModal"
                                                    onclick="showAssignedUsers('{{ $key }}')">
                                                    <i class="fa fa-eye view-icon" style="color:blue"></i>
                                                </a>
                                            @endif
                                        </td>
                                    </tr>
                                    <tr class="assigned-users-row" id="assignedUsersRow{{ $key }}"
                                        style="display: none;">
                                        <td colspan="6">
                                            <table class="table">
                                                <tbody>
                                                    @foreach ($material->assignedUsers as $userKey => $user)
                                                        <tr>
                                                            ...
                                                            @if ($material->materialType->materialCategory->has_usage_limit)
                                                                <td style="color: red;">
                                                                    {{ $user->pivot->created_at->addDays($user->pivot->usage_limit)->format('Y-m-d') }}
                                                                </td>
                                                            @endif
                                                        </tr>
                                                    @endforeach
            ...
            <!-- Modal -->
            <div class="modal fade" id="assignedUsersModal" tabindex="-1" role="dialog"
                aria-labelledby="assignedUsersModalLabel">
